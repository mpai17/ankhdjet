#!/usr/bin/env bash
# Driver script for running the LibreLane Classic flow on cirom_chip_digital.
#
...
#
# Run from repo root:
#   bash librelane/cirom_chip_digital/run_librelane.sh [run-tag]
...
exec uv run python -m librelane \
    ...
    --pdk sky130A \
    --run-tag "$RUN_TAG" \
    librelane/cirom_chip_digital/config.json
